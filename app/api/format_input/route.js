    ...
    try {
      ...
      if (parsedInput) {
        ...
        try {
          ...
          const endpoint = process.env.Vercel_Base_Url + "/firebase_ops/";
      ...
      messages: [
        {
          role: "system",
          content:
            "You will convert the following prompt to a simple command which will contain the command of what the user is trying to do ( either add delete search or update) and the item to do the command on. Return only a valid json string and nothing else (no markup or anything, as this is going to be converted. That means no (```, or invalid characters that will lead to errors), with keys being 0,1,2. 0 will contain the command, 1 the value or amount to work with (if its spelled out put it as a number), and 2 the item the user is requesting us to operate with, if the prompt doesnt have a quantity just put an empty string, make sure to return a valid json string.",
        },
        {
          role: "user",
          content: request,
